...
def process_image(input_fn, input_cix):
    ...
    for increment in inc_list:
        ...
        def sum_crop_scores(x):
            res = 0
            for map in heatmaps:
                res += map[x[0], x[1], input_cix]
            return res

        ordpositions = sorted(positions, key=sum_crop_scores)
        best_crop_ix = ordpositions[-1]
        best_crop_score = sum_crop_scores(best_crop_ix) / 4

        results.append((increment, heatmaps[-1].shape[0:2], best_crop_ix, best_crop_score, maxcn))
...
# "dataset-ethz101food/train/cup_cakes/46500.jpg", "il baklava incartato"
for i_folder, class_folder in enumerate(class_folders[0:folder_to_scan]):
    ...
    for i_instance, instance in enumerate(instances[0:instances_per_folder]):
        ...
        results = process_image(filename, class_name_to_idx(class_folder))
        ...
        fcn_window = sum(initial_sizes) // len(initial_sizes)
        factor = fcn_window / (fcn_window + results[ixMax][0])
        rect_dim = int(fcn_window / factor)
        coordh = traslation(results[ixMax][2][0], factor)
        coordw = traslation(results[ixMax][2][1], factor)

        print("Max confidence", results[ixMax][3], "found at input size", fcn_window+results[ixMax][0], "(original size " + str(img.shape[0:2]) + "):",
              "heatmap cell", (results[ixMax][2][0], results[ixMax][2][1]), "in range [" + str(results[ixMax][1][0]) + ", " + str(results[ixMax][1][1]) + "] ->",
              "relative img point", (coordh, coordw), "in range [" + str(img.shape[0])+ ", " + str(img.shape[1]) + "]; crop_dim", rect_dim)

        fig, ax = plt.subplots(1)
        ax.set_title("Ensemble crop")
        ax.imshow(img / 255.)
